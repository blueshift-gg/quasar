...
impl<'input> ParseAccounts<'input> for InitEscrow {
    type Bumps = InitEscrowBumps;
...
unsafe impl<'input> quasar_lang::traits::ParseAccountsUnchecked<'input> for InitEscrow {
    #[inline(always)]
...
impl InitEscrow {
    ...
    pub unsafe fn parse_accounts(
        ...
        {
            const __EXPECTED: u32 = {
                const __S: bool = <Program<
                    SystemProgram,
                > as quasar_lang::account_load::AccountLoad>::IS_SIGNER;
                const __E: bool = <Program<
                    SystemProgram,
                > as quasar_lang::account_load::AccountLoad>::IS_EXECUTABLE;
    ...
    pub unsafe fn parse_direct_with_instruction_data_unchecked(
        ...
        __ix_data: &[u8],
        __program_id: &quasar_lang::prelude::Address,
    ) -> Result<(Self, InitEscrowBumps), ProgramError> {
...
macro_rules! __init_escrow_instruction {
    ($struct_name:ident, [$($disc:expr),*], { $($arg_name:ident : $arg_ty:ty),* }) => {
        pub struct $struct_name { pub payer : quasar_lang::prelude::Address, pub escrow :
