...
pub const SIMPLE_ACCOUNT_ACCOUNT_DISCRIMINATOR: &[u8] = &[10];
...
// written below.
unsafe impl<C: ConfigCore> SchemaWrite<C> for SimpleAccount
where
    ...
    u8: SchemaWrite<C, Src = u8>,
{
    ...
    fn write(mut writer: impl Writer, src: &Self) -> WriteResult<()> {
        writer.write(SIMPLE_ACCOUNT_ACCOUNT_DISCRIMINATOR)?;
...
unsafe impl<'de, C: ConfigCore> SchemaRead<'de, C> for SimpleAccount
where
