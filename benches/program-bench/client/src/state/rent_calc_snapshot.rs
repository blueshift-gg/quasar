...
pub const RENT_CALC_SNAPSHOT_ACCOUNT_DISCRIMINATOR: &[u8] = &[19];
...
// written below.
unsafe impl<C: ConfigCore> SchemaWrite<C> for RentCalcSnapshot
where
    u64: SchemaWrite<C, Src = u64>,
{
    ...
    fn size_of(src: &Self) -> WriteResult<usize> {
        Ok(1
            + <u64 as SchemaWrite<C>>::size_of(&src.min_balance)?)
    }

    fn write(mut writer: impl Writer, src: &Self) -> WriteResult<()> {
        writer.write(RENT_CALC_SNAPSHOT_ACCOUNT_DISCRIMINATOR)?;
        <u64 as SchemaWrite<C>>::write(writer.by_ref(), &src.min_balance)?;
        Ok(())
    }
}
...
unsafe impl<'de, C: ConfigCore> SchemaRead<'de, C> for RentCalcSnapshot
where
    u64: SchemaRead<'de, C, Dst = u64>,
{
    ...
    fn read(mut reader: impl Reader<'de>, dst: &mut MaybeUninit<Self>) -> ReadResult<()> {
        ...
        dst.write(Self {
            min_balance: <u64 as SchemaRead<'de, C>>::get(reader.by_ref())?,
        });
        Ok(())
    }
}
