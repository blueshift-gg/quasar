...
pub const SPACE_TEST_ACCOUNT_ACCOUNT_DISCRIMINATOR: &[u8] = &[11];
...
// written below.
unsafe impl<C: ConfigCore> SchemaWrite<C> for SpaceTestAccount
where
    ...
    u8: SchemaWrite<C, Src = u8>,
{
    ...
    fn write(mut writer: impl Writer, src: &Self) -> WriteResult<()> {
        writer.write(SPACE_TEST_ACCOUNT_ACCOUNT_DISCRIMINATOR)?;
...
unsafe impl<'de, C: ConfigCore> SchemaRead<'de, C> for SpaceTestAccount
where
