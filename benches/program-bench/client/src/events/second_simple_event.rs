...
pub const SECOND_SIMPLE_EVENT_DISCRIMINATOR: &[u8] = &[7];
...
// written below.
unsafe impl<C: ConfigCore> SchemaWrite<C> for SecondSimpleEvent
where
    u64: SchemaWrite<C, Src = u64>,
{
    ...
    fn write(mut writer: impl Writer, src: &Self) -> WriteResult<()> {
        writer.write(SECOND_SIMPLE_EVENT_DISCRIMINATOR)?;
...
unsafe impl<'de, C: ConfigCore> SchemaRead<'de, C> for SecondSimpleEvent
where
