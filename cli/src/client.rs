...
const DEFAULT_TARGETS: &[&str] = &["kit", "web3"];
...
pub fn run(command: ClientCommand) -> CliResult {
    ...
    println!(
        "  {}",
        style::success(&format!("Clients generated: {}", targets.join(", ")))
    );
    Ok(())
}
...
pub(crate) fn prepare_clients(
    ...
    let requested = languages
        ...
        .map(|language| match *language {
            "kit" => Ok(ClientLanguage::Kit),
            "web3" => Ok(ClientLanguage::Web3),
            "python" => Ok(ClientLanguage::Python),
            "go" => Ok(ClientLanguage::Go),
            "c" => Ok(ClientLanguage::C),
            other => Err(CliError::message(format!(
                "unknown client target: '{other}'. Options: kit, web3, python, go, c"
            ))),
        })
    ...
    if requested.contains(&ClientLanguage::Kit) {
    ...
    if requested.contains(&ClientLanguage::Web3) {
...
mod tests {
    ...
    fn hostile_identity_cannot_escape_clients_root() {
        ...
        let error = generate_clients(&idl, &["kit"], root.path()).unwrap_err();
    ...
    fn invalid_later_language_preserves_existing_outputs() {
        let root = tempdir().unwrap();
        let web3 = root.path().join("web3/vault/client.ts");
        fs::create_dir_all(web3.parent().unwrap()).unwrap();
        fs::write(&web3, "existing").unwrap();

        let error =
            generate_clients(&minimal_idl("vault"), &["web3", "bogus"], root.path()).unwrap_err();
    ...
    fn successful_generation_is_byte_for_byte_deterministic() {
        ...
        let languages = ["kit", "web3", "python", "go", "c"];
