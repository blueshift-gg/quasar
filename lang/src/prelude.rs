...
// resolves the runtime crate by its dependency name (see
// `derive/src/krate.rs`), a future rename to `quasar` would not require
...
// every program's generated code, so the split is documentation-only:
// `internal` re-exports the same items for readers and for explicit imports.
