...
//! [`Owners`](crate::traits::Owners) set (e.g. SPL Token and Token-2022).
//!
//! `InterfaceAccount<T>` is `#[repr(transparent)]` over
//! [`AccountView`](solana_account_view::AccountView) and is constructed only
//! after owner and data validation, so every `Deref` re-relies on that
//! construction invariant to reach the account data through a pointer cast.
