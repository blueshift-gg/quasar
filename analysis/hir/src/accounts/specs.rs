...
//! emit `AccountBehavior` trait calls.
...
pub enum LoweredValue {
    ...
    FieldView(Ident),
    /// `field.as_ref().map(|v| v.to_account_view())`: optional field reference.
